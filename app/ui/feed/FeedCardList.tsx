...
const FeedCardList = ({ data }: { data: Feed_Post }) => {
  ...
  const voteAction = async (value: number) => {
    ...
    try {
      ...
      console.log('FeedCardList Vote Error:', error)
    }
  }
    ...
    <div>
      ...
      >
        <div className="flex gap-2 items-center mb-2">
          ...
          />
          <Link href={`/dashboard/feed/${data.categoryName}`}>
            <h6 className="text-xs hover:font-semibold">
              /{data.categoryName}
            </h6>
          </Link>
          <h6 className="text-xs">{data.username}</h6>
          <h6 className="text-xs">{prettifyToYMDorTime(data.updated_at)}</h6>
        </div>
        <Link href={`/dashboard/feed/${data.categoryName}/${data.identifier}`}>
          ...
          <p
            ...
            truncate rounded-3xl text-sm py-2`}
          >
            {data.body}
          </p>
        </Link>
...
export const FeedCardList_Skeleton = () => {
  return (
    <div
    ...
    >
      ...
      <div className="flex items-center justify-center truncate rounded-3xl bg-white px-4 py-8">
